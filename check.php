...
function function_support(&$func_items) {
	...
	foreach($func_items as $item) {
		...
		if ($item == "close_curl") {
			...
			if (!$status) {
				$func_str .= " <span style=\"color:green\">不支持Zend，意味着不能使用 “专业版”。 php5.2.x请安装Zend Optimizer , php5.3.x请安装Zend Guard Loader</span>";
			} else {
				$func_str .= ( version_compare($version, '3.3', '<') ) ? " <span style=\"color:red\">版本太低，php5.2.x请升级到3.3.0或以上版本，否则不能使用 “专业版”</span>" : '';
			}
			$func_str .= "</td>\n";
		} else {
			$func_str .= "<td>$item()</td>\n";
