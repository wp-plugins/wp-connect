...
define('WP_CONNECT_VERSION', '1.9.24');
...
if ($wptm_version && $wptm_version != WP_CONNECT_VERSION) {
	...
	if (version_compare($wptm_version, '1.9.24', '<'))
...
function wp_connect_set_session_init() {
	if (!empty($_GET['oauth_token']) || !empty($_GET['user_id']) || defined( 'IS_PROFILE_PAGE' ) || $_GET['page'] == "wp-connect" || $_GET['action'] == "profile") {
		do_action('connect_init');
	} 
	if ( !is_user_logged_in() && ( !empty($_GET['go']) || !empty($_GET['login']) ) ) { 
		do_action('connect_init');
		$_SESSION['wp_url_back'] = !empty($_GET['redirect_url']) ? $_GET['redirect_url'] : $_SERVER['HTTP_REFERER'];
	} 
}
...
function wp_redirect_url() {
	...
	if ( is_home() ) {
		...
		return "&redirect_url=" . urlencode(get_bloginfo('wpurl'));
	} else {
...
function wp_connect_warning() {
	...
	if (current_user_can('manage_options')) {
		...
		if ($wptm_tips || version_compare($wp_version, '3.0', '<') || (donate_version($wp_connect_advanced_version) && WP_CONNECT_ADVANCED_VERSION != '1.4.3') || (($wptm_options || $wptm_connect) && !$wptm_version) || (!$wptm_connect && !$wptm_options)) {
      ...
      <form method="post" action="options-general.php?page=wp-connect#connect">
        ...
        <table class="form-table">
          ...
          <tr>
            ...
            <td><label><input name="qqlogin" type="checkbox" value="1" <?php checked($wptm_connect['qqlogin']);?><?php echo $disabled;?> />QQ登录</label>
			  <label><input name="sina" type="checkbox" value="1" <?php checked(!$wptm_connect || $wptm_connect['sina']);?> />新浪微博</label>
              <label><input name="qq" type="checkbox" value="1" <?php checked(!$wptm_connect || $wptm_connect['qq']);?> />腾讯微博</label>
			  <label><input name="taobao" type="checkbox" value="1" <?php checked($wptm_connect['taobao']);?><?php echo $disabled;?> />淘宝网</label>
			  <label><input name="baidu" type="checkbox" value="1" <?php checked($wptm_connect['baidu']);?><?php echo $disabled;?> />百度</label>
              <label><input name="sohu" type="checkbox" value="1" <?php checked(!$wptm_connect || $wptm_connect['sohu']);?> />搜狐微博</label>
              <label><input name="netease" type="checkbox" value="1" <?php checked(!$wptm_connect || $wptm_connect['netease']);?> />网易微博</label><br />
              <label><input name="renren" type="checkbox" value="1" <?php checked(!$wptm_connect || $wptm_connect['renren']);?> />人人网</label>
              <label><input name="kaixin001" type="checkbox" value="1" <?php checked($wptm_connect['kaixin001']);?><?php echo $disabled;?> />开心网</label>
              <label><input name="douban" type="checkbox" value="1" <?php checked(!$wptm_connect || $wptm_connect['douban']);?> />豆瓣</label>
              <label><input name="tianya" type="checkbox" value="1" <?php checked(!$wptm_connect || $wptm_connect['tianya']); ?> />天涯</label>
			  <label><input name="msn" type="checkbox" value="1" <?php checked($wptm_connect['msn']);?><?php echo $disabled;?> />MSN</label>
			  <label><input name="google" type="checkbox" value="1" <?php checked($wptm_connect['google']);?><?php echo $disabled;?> />谷歌</label>
			  <label><input name="yahoo" type="checkbox" value="1" <?php checked($wptm_connect['yahoo']);?><?php echo $disabled;?> />雅虎</label>
			  <label><input name="twitter" type="checkbox" value="1" <?php checked(!$wptm_connect || $wptm_connect['twitter']);?> />Twitter</label>
