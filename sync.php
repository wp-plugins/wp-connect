...
function connect_denglu_update() {
	global $wpdb,$wptm_basic;
	@ini_set("max_execution_time", 180);
	$userids = $wpdb -> get_col($wpdb -> prepare("SELECT $wpdb->users.ID FROM $wpdb->users ORDER BY ID ASC"));
	foreach ($userids as $uid) {
		$user = get_userdata($uid);
		$ret = array_filter(array(//'1' => ($user -> googleid) ? array('mediaID' => '1', 'mediaUID' => $user -> user_login, 'email' => $user -> user_email) : '',

				'2' => ($user -> mmid) ? '' : (($user -> msnid) ? array('mediaID' => '2', 'mediaUID' => $user -> msnid, 'email' => $user -> user_email) : ''),
				...
				'6' => ($user -> nmid) ? '' : (($user -> ntid) ? array('mediaID' => '6', 'mediaUID' => $user -> neteaseid, 'profileImageUrl' => $user -> ntid, 'oauth_token' => ifac($user -> login_netease[0], $user -> tdata['tid'] == 'ntid', $user -> tdata['oauth_token']), 'oauth_token_secret' => ifac($user -> login_netease[1], $user -> tdata['tid'] == 'ntid', $user -> tdata['oauth_token_secret'])) : ''),
				...
				//'12' => ($user -> yahooid) ? array('mediaID' => '12', 'mediaUID' => $user -> user_login, 'email' => $user -> user_email):'',
        ...
        // 更新数据库
		$ids = array('4' => 'tqq', '5' => 'sohu', '6' => 'netease', '7' => 'renren', '8' => 'kaixin');
	...
	$content = json_encode(array_filter($result));
	//return $content;
	require(dirname(__FILE__) . "/class/Denglu.php");
    ...
    // $out = array('1' => array('1' => '13768191'), '3' => array('12' => '13768194'),'7' => array('12' => '13768198'));
	if (is_array($output)) {
		...
		$basic = get_option("wptm_basic");
		$basic['denglu'] = 1;
		update_option("wptm_basic", $basic);
	}
}
/*
 * 插件页面
