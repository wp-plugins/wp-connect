...
if($errors){
    ...
    echo ($_POST['bind'])?"已有帐号？绑定我的帐号":"这是您第一次登录，请先完善注册信息。";
    echo ($user[0][0] != "qqtid")?" <a href='save.php?do=login'>跳过这一步</a>" : "";
    echo "</p>";
}
$user_login = (isset($_POST['login'])) ? $_POST['login'] : $user[0][2];
$user_email = (in_array($user[0][0], array('ntid','tbtid','gtid','ytid','mtid'))) ? $user[1] : $_POST['email'];
